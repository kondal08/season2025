...
public class GenericArmSystemIOSparkFlex implements GenericArmSystemIO {
    ...
    public GenericArmSystemIOSparkFlex(
            int[] id, int currentLimitAmps, double restingAngle, boolean invert, boolean brake, double reduction, double kP, double kI, double kD) {
        ...
        for(int i = 0; i<id.length; i++) {
            ...
            if(i == 0) motors[i].configure(config, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);

            else motors[i].configure(new SparkFlexConfig().follow(motors[0]), ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
        }
