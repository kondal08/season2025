...
public class LimelightHelpers {
  ...
  /**
   * @param targetDistance distance from camera (in meters)
   * @param tx yaw offset of crosshair from target (in degrees)
   * @return target translation from camera (in meters)
   */
  public static Transform2d cameraToTargetTranslationOffset(double targetDistance, double tx) {
    var yawRadians = Rotation2d.fromDegrees(tx);
    return new Transform2d(
        yawRadians.getCos() * targetDistance,
        yawRadians.getSin() * targetDistance,
        new Rotation2d());
  }

  /**
   * @param cameraHeight distance from lens of camera to floor in meters
   * @param cameraPitch pitch of camera (in degrees)
   * @param targetHeight distance from floor to center of target in meters
   * @param ty pitch offset of crosshair from target (in degrees)
   * @return target distance from camera (in meters)
   */
  public static double targetToCameraDistance(
      double cameraHeight, double cameraPitch, double targetHeight, double ty) {
    double angleToGoalRadians = (cameraPitch + ty) * (Math.PI / 180.0);
